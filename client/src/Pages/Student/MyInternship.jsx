...
export default function MyInternship() {
  ...
  const handleOfferLetterUpload = async (e) => {
    ...
    try {
      ...
      setFormData(prev => ({ ...prev, offer_letter_url: data.publicUrl }));
    } catch (err) {
              ...
              {internships.map((item) => (
                <div key={item.id} className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm flex items-center justify-between hover:shadow-md transition-all group">
                  <div className="flex items-center gap-5">
                    <div className="w-12 h-12 bg-blue-50 text-[#0047AB] rounded-2xl flex items-center justify-center"><Building2 size={24} /></div>
                    <div>
                      <h3 className="font-bold text-slate-800 text-lg mb-1">{item.company_name}</h3>
        ...
        <div className="bg-white w-full max-w-6xl h-[85vh] rounded-[32px] shadow-2xl overflow-hidden flex flex-col relative animate-in slide-in-from-bottom-8">
          ...
          <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
               ...
               {selectedIntern.offer_letter_url ? (
                 <a href={selectedIntern.offer_letter_url} target="_blank" className="flex items-center gap-3 p-4 border rounded-2xl text-sm font-bold text-slate-700 hover:bg-slate-50">
                    <CheckCircle2 className="text-emerald-500" /> View Offer Letter
                 </a>
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-8 lg:p-12 overflow-y-auto">
      <div className="max-w-2xl mx-auto bg-white p-10 rounded-[32px] border shadow-xl relative">
        ...
        <form onSubmit={handleAddInternship} className="space-y-6">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Signed Offer Letter*</label>
            <div className="relative border-2 border-dashed border-slate-200 p-8 rounded-2xl flex flex-col items-center group hover:border-[#0047AB]">
              {uploading ? <Loader2 className="animate-spin text-[#0047AB]" /> : <Upload className="text-slate-300 group-hover:text-[#0047AB]" />}
              <input type="file" onChange={handleOfferLetterUpload} className="absolute inset-0 opacity-0 cursor-pointer" />
              <span className="text-[10px] font-bold text-slate-400 mt-2">{formData.offer_letter_url ? "File Ready ✅" : "Upload PDF"}</span>
            </div>
          </div>

          <button type="submit" className="w-full bg-[#0047AB] text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg">
